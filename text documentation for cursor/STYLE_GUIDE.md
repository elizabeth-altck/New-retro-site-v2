...
- **Текст макета (дословно по смыслу):** «Персонаж этого сайта — улитка ахатина **Жанна**. У нее есть подруга **Анна**. На **этой странице** ты можешь ее погладить или покормить. Сейчас ты можешь воспользоваться **виджетом**.»
- Ссылки: `zhanna.html`, `anna.html`, `tamagotchi.html`, `#tama-widget` (виджет на главной).

---

# Публичный текст (обязательно)

**Всё, что видит гость на сайте, — только для гостя.**

Запрещено в публичной вёрстке (шапка, подвал, подписи блоков, aria-label для видимого текста, статусы, подсказки):

- внутренние названия блоков и форматов («scrollport», «site TODO», «v1», «.html» в тексте);
- пометки разработчику («что добавлю на сайт», «позже свяжем», «endpoint», имена файлов и ключей);
- технический сленг, который не звучит как часть мира сайта.

**Допустимо:** in-world имена (`spam.exe`, `hotline.txt`, `music.exe`) — если это персонаж/объект поля, а не заметка в блокноте разработчика.

**Список дел на главной** — публичная фича (как todo у neocities): интересен и тебе, и гостю. Подпись блока — «поле · список дел · листай ↓», не «scrollport».

**Документация для Cursor** (`text documentation for cursor/`) — dev-зона; правило не отменяет заметки там.

**Следующий проход (фаза 5–6):** убрать оставшиеся dev-подписи по всему сайту (gallery, diary, footer, meta-строки комнат).
